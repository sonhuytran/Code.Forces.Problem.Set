...
public class P26A_AlmostPrime {
	...
	public static void main(String[] args) {
		...
		int maxPrime = 240, count = 0;
		Primes primes = new Primes(maxPrime);
		int[] primeNumbers = primes.getPrimes();

		for (int i = 1; i <= maxPrime && primeNumbers[i] <= n; i++) {
			for (int j = i + 1; j <= maxPrime
					&& primeNumbers[j] * primeNumbers[i] <= n; j++) {
				for (int prime1 = primeNumbers[i]; prime1 <= n; prime1 *= primeNumbers[i]) {
					for (int prime2 = primeNumbers[j]; prime2 * prime1 <= n; prime2 *= primeNumbers[j]) {
						count++;
					}
				}
			}
